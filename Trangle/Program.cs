...
public class Triangle
{
          public static void Main(string[] args)
        {
            ...
            var triangleInfo = Triangle.GetTriangleInfo(sideA, sideB, sideC);

            Console.WriteLine("Тип треугольника: " + triangleInfo.Item1);

            Console.WriteLine("Координаты вершин:");
            foreach (var vertex in triangleInfo.Item2)
            {
    ...
    public static (string, List<(int, int)>) GetTriangleInfo(string sideA, string sideB, string sideC)
    {
        ...
        bool convertResultA = float.TryParse(sideA, NumberStyles.Float, CultureInfo.CreateSpecificCulture("ru-RU"), out float a);
        bool convertResultB = float.TryParse(sideB, NumberStyles.Integer, CultureInfo.CreateSpecificCulture("ru-RU"), out float b);
        bool convertResultC = float.TryParse(sideC, NumberStyles.Float, CultureInfo.CreateSpecificCulture("en-US"), out float c);
    ...
    private static string GetTriangleType(float a, float b, float c)
    {
        ...
        /// <summary>
        /// Вычисление координат трех вершин треугольника:
        /// Вершина А - лежит напротив стороны с длиной а,
        /// Вершина B - лежит напротив стороны с длиной b,
        /// Вершина C - лежит напротив стороны с длиной с.
        /// </summary>
        /// <param name="a">Длина стороны а</param>
        /// <param name="b">Длина стороны b</param>
        /// <param name="c">Длина стороны с</param>
        /// <returns>Список кортежей (координат вершин А, B и C)</returns>
